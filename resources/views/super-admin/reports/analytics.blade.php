...
<div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $stats['new_companies_this_month'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Companies by Status</h3>
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-blue-600 h-2 rounded-full" style="width: {{ ($status->count / $stats['total_companies']) * 100 }}%"></div>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Companies by Plan</h3>
            <div class="space-y-3">
                @foreach($stats['companies_by_plan'] as $plan)
                <div>
                    <div class="flex justify-between text-sm mb-1">
                        <span class="text-gray-600 capitalize">{{ $plan->subscription_plan }}</span>
                        <span class="font-medium text-gray-900">{{ $plan->count }}</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="bg-green-600 h-2 rounded-full" style="width: {{ ($plan->count / $stats['total_companies']) * 100 }}%"></div>
                    </div>
                </div>
                @endforeach
            </div>
