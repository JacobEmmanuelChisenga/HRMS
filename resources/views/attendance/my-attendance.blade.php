...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-3">
            @php
                ...
                $todayRecord = $records->firstWhere('date', $today->format('Y-m-d'));
            @endphp
            ...
            @if($todayRecord && $todayRecord->clock_in && !$todayRecord->clock_out)
                <form action="{{ route('attendance.clock-out') }}" method="POST">
                    @csrf
                    <button type="submit" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span>Clock Out</span>
    ...
    <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg shadow-lg p-6 text-white">
        ...
        <div class="grid grid-cols-3 gap-4">
            <div>
                <p class="text-sm opacity-90">Clock In</p>
                <p class="text-2xl font-bold">{{ $todayRecord->clock_in ? \Carbon\Carbon::parse($todayRecord->clock_in)->format('h:i A') : 'Not clocked in' }}</p>
            </div>
            <div>
                <p class="text-sm opacity-90">Clock Out</p>
                <p class="text-2xl font-bold">{{ $todayRecord->clock_out ? \Carbon\Carbon::parse($todayRecord->clock_out)->format('h:i A') : 'Not clocked out' }}</p>
            </div>
            <div>
                <p class="text-sm opacity-90">Status</p>
                <p class="text-2xl font-bold">{{ ucfirst(str_replace('_', ' ', $todayRecord->status)) }}</p>
            </div>
        </div>
    </div>
    @endif
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $record->date->format('M d, Y') }}</td>
