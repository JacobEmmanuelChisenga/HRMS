...
<head>
    ...
    <title>{{ $company ? ($company->name . ' - Login') : 'Login' }} - {{ config('app.name', 'HRMS') }}</title>
        ...
        <div class="hidden lg:flex lg:w-1/2 gradient-bg relative overflow-hidden">
            ...
            <div class="relative z-10 flex flex-col justify-between p-12 text-white">
                <div>
                    ...
                    @if($company && $company->login_page_image)
                        <div class="mb-8">
                            <img src="{{ Storage::url($company->login_page_image) }}" 
                                 alt="{{ $company->name }}" 
                                 class="rounded-2xl shadow-2xl w-full max-w-md">
                        </div>
                    @endif
                    
                    @if($company && ($company->login_page_title || $company->login_page_subtitle))
                        <div class="space-y-4">
                            @if($company->login_page_title)
                                <h1 class="text-4xl font-bold leading-tight">{{ $company->login_page_title }}</h1>
                            @else
                                <h1 class="text-4xl font-bold leading-tight">Welcome Back</h1>
                            @endif
                            
                            @if($company->login_page_subtitle)
                                <p class="text-xl text-white/90 leading-relaxed">{{ $company->login_page_subtitle }}</p>
                            @else
                                <p class="text-xl text-white/90 leading-relaxed">Sign in to access your HRMS dashboard and manage your workforce efficiently.</p>
                            @endif
                        </div>
                    @else
                        <div class="space-y-4">
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-center p-8">
            <div class="w-full max-w-md">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <!-- Submit Button -->
                    <button type="submit" class="w-full btn-primary text-white font-semibold py-3 px-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
                        Sign In
                    </button>
